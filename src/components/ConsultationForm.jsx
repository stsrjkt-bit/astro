...
import { User, Mail, GraduationCap, MessageSquare, Send, CheckCircle2, AlertCircle, Loader2, Calendar, Clock, Sparkles } from 'lucide-react';

// 予約枠の定義（13:00〜20:00開始）
const TIME_SLOTS = [
  "13:00〜13:55",
  "14:00〜14:55",
  "15:00〜15:55",
    ...
    return (
      <div className="bg-white border border-[#009DE0]/20 rounded-2xl p-10 text-center max-w-2xl mx-auto my-10 shadow-xl animate-fade-in relative overflow-hidden">
        ...
        <div className="bg-[#F8FAFC] p-6 rounded-xl inline-block text-left mb-8 border border-[#009DE0]/10">
          <p className="text-sm text-[#334455] font-bold mb-2 flex items-center gap-2"><Calendar size={16} className="text-[#009DE0]"/>ご予約日時</p>
          <p className="text-xl text-[#009DE0] font-bold">{selectedDate} {selectedTime}</p>
        </div>
        <br />
        <button 
      ...
      <div className="relative z-10 p-8 md:p-12">
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          ...
          <div className="bg-[#F8FAFC] p-6 md:p-8 rounded-2xl border border-[#334455]/5 shadow-sm relative overflow-hidden">
            ...
            <div className="mb-8">
              <p className="text-base text-[#334455] mb-4 font-bold flex items-center gap-2">
                <span className="bg-[#334455] text-white rounded-full w-6 h-6 flex items-center justify-center text-sm">1</span>
                日程を選ぶ (土曜開催)
              </p>
                ...
                {saturdays.map((date, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {!isSelected && <div className="absolute inset-0 bg-[#009DE0]/5 scale-0 group-hover:scale-100 transition-transform origin-center rounded-xl"></div>}
                    </button>
                  );
                })}
              </div>
            ...
            {/* 時間選択 */}
            <div className={`transition-all duration-500 ${selectedDate ? 'opacity-100' : 'opacity-50 pointer-events-none blur-sm'}`}>
              <p className="text-base text-[#334455] mb-4 font-bold flex items-center gap-2">
                <span className="bg-[#334455] text-white rounded-full w-6 h-6 flex items-center justify-center text-sm">2</span>
                時間を選ぶ
              ...
              <div className="mt-8 p-6 bg-[#009DE0]/5 border-2 border-[#009DE0] rounded-xl flex flex-col md:flex-row items-center justify-center gap-4 animate-fade-in">
                ...
                <div className="text-center md:text-left">
                  ...
                  <span className="text-[#334455] font-bold text-xl">
                    <span className="text-[#009DE0]">{selectedDate}</span> の <span className="text-[#009DE0]">{selectedTime}</span>
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="space-y-8 px-2">
            {/* 入力フォーム項目 (名前, 学年, メール, メッセージ) */}
            <div className="grid md:grid-cols-2 gap-8">
              <div className="group">
                <label htmlFor="name" className="flex items-center gap-2 text-sm font-bold text-[#334455] mb-2 group-focus-within:text-[#009DE0] transition-colors">
                  <User size={20} className="text-[#009DE0]" />
                  お名前 <span className="text-[#EA5514] text-xs ml-1">*必須</span>
                </label>
                <input type="text" name="name" id="name" required placeholder="例：佐藤 太郎" className="w-full px-4 py-4 rounded-xl bg-[#F8FAFC] border-2 border-[#334455]/10 focus:bg-white focus:border-[#009DE0] outline-none transition-all shadow-sm" />
              </div>
              <div className="group">
                <label htmlFor="grade" className="flex items-center gap-2 text-sm font-bold text-[#334455] mb-2 group-focus-within:text-[#009DE0] transition-colors">
                  <GraduationCap size={20} className="text-[#009DE0]" />
                  学年 <span className="text-[#EA5514] text-xs ml-1">*必須</span>
                </label>
                <div className="relative">
                  <select name="grade" id="grade" required defaultValue="" className="w-full px-4 py-4 rounded-xl bg-[#F8FAFC] border-2 border-[#334455]/10 focus:bg-white focus:border-[#009DE0] outline-none appearance-none cursor-pointer shadow-sm">
            ...
            <div className="group">
              <label htmlFor="email" className="flex items-center gap-2 text-sm font-bold text-[#334455] mb-2 group-focus-within:text-[#009DE0] transition-colors">
                <Mail size={20} className="text-[#009DE0]" />
                メールアドレス <span className="text-[#EA5514] text-xs ml-1">*必須</span>
              </label>
              <input type="email" name="email" id="email" required placeholder="example@email.com" className="w-full px-4 py-4 rounded-xl bg-[#F8FAFC] border-2 border-[#334455]/10 focus:bg-white focus:border-[#009DE0] outline-none transition-all shadow-sm" />
            </div>
            <div className="group">
              <label htmlFor="message" className="flex items-center gap-2 text-sm font-bold text-[#334455] mb-2 group-focus-within:text-[#009DE0] transition-colors">
                <MessageSquare size={20} className="text-[#009DE0]" />
                今の悩み・相談したいこと <span className="text-[#EA5514] text-xs ml-1">*必須</span>
              </label>
              <textarea name="message" id="message" rows="5" required placeholder="「数学のテストが30点台で...」など、簡単で構いませんのでご記入ください。" className="w-full px-4 py-4 rounded-xl bg-[#F8FAFC] border-2 border-[#334455]/10 focus:bg-white focus:border-[#009DE0] outline-none resize-y transition-all shadow-sm"></textarea>
            </div>
          </div>
          ...
          <div className="pt-6">
            <button type="submit" disabled={isSubmitting} className={`w-full font-bold py-5 rounded-full shadow-xl transition-all duration-300 flex justify-center items-center text-xl gap-3 ${isSubmitting ? 'bg-gray-300 cursor-not-allowed text-white shadow-none' : 'bg-gradient-to-r from-[#009DE0] to-[#008ac4] hover:from-[#008ac4] hover:to-[#0078b0] text-white hover:shadow-2xl hover:-translate-y-1'}`}>
              {isSubmitting ? <><Loader2 className="animate-spin" size={28} />送信中...</> : <><Send size={24} />この内容で予約を申し込む</>}
            </button>
          </div>
        </form>
      </div>
