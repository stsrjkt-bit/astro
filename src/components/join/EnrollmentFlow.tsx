...
const ChecklistItem: React.FC<ChecklistItemProps> = ({ text, isChecked, onToggle, isStepCompleted }) => {
  return (
    ...
    >
      {/* チェックボックスアイコン */}
      <div className={`checkbox-icon flex-shrink-0 mt-0.5 w-6 h-6 rounded border-2 flex items-center justify-center transition-all duration-300 
        ${isChecked 
          ? (isStepCompleted ? 'bg-slate-400 border-slate-400' : 'bg-[#06C755] border-[#06C755] shadow-sm') 
      ...
      {/* テキスト */}
      <span className={`text-sm md:text-base leading-relaxed transition-all duration-300 ${isChecked ? 'text-slate-400 line-through decoration-slate-400' : 'text-[#334455]'}`}>
        {text}
      </span>
    </li>
  );
...
const StepCard: React.FC<{ step: Step }> = ({ step }) => {
  const [checkedState, setCheckedState] = useState(
    new Array(step.checklist.length).fill(false)
  );
  ...
  return (
    ...
    >
      ...
      {/* 装飾: 背景の数字（色を少し青寄りに調整） */}
      <div className={`absolute -right-4 -top-6 text-[120px] font-serif font-bold pointer-events-none select-none z-0 transition-colors duration-500
        ${isCompleted ? 'text-slate-200' : 'text-blue-50/80'}`}>
        {step.number}
      </div>

      {/* 完了バッジ */}
      <div className={`absolute top-4 right-4 md:right-8 z-20 transition-all duration-500 transform
      ...
      <div className={`relative z-10 transition-opacity duration-500 ${isCompleted ? 'opacity-60' : 'opacity-100'}`}>
        ...
        <div className="flex items-center gap-4 mb-6">
          <div className={`flex-shrink-0 w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center text-lg md:text-xl font-bold shadow-sm font-serif transition-colors duration-500
            ${isCompleted 
          ...
          <h2 className={`text-xl md:text-2xl font-bold transition-colors duration-500 ${isCompleted ? 'text-slate-500' : 'text-[#334455]'}`}>
            {step.title}
          </h2>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="text-center mb-12 md:mb-16">
          <div className="inline-block mb-4">
            <span className="bg-blue-100 text-[#009DE0] text-xs font-bold px-3 py-1 rounded-full tracking-wider">GUIDE</span>
          </div>
          ...
          <p className="text-sm md:text-base leading-relaxed text-[#334455]/80 max-w-2xl mx-auto bg-white/50 backdrop-blur-sm p-4 rounded-xl border border-blue-50 shadow-sm">
            さとう数理塾では、月ごとに無料体験を受け付けています。<br className="hidden md:inline" />
            <strong className="text-[#009DE0] bg-blue-50 px-1 py-0.5 rounded border border-blue-100">最大1ヶ月まで無料</strong>で通えるので、<br className="md:hidden" />
        ...
        <div className="mt-16 text-center">
          <a href="/" className="text-[#009DE0] hover:underline hover:text-[#008ac4] text-sm font-medium inline-flex items-center gap-1 cursor-pointer bg-white px-6 py-3 rounded-full shadow-sm hover:shadow-md transition-all">
